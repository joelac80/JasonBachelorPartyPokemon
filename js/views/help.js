...
(function () {
  ...
  const GROUPS = [
    ...
    },
    {
      title: "🎯 Games to play",
      items: [
        { r: "safari", e: "🔴", t: "Pokédex Safari", d: "Find a wild one, then raise the odds the CLASSIC way: 🍓 berries (start +8%, 4% flee) and 🪨 rocks (start +20%, 12% flee) — but every repeat throw is worth LESS (×0.65 each) and every aid riles it MORE (+5% flee per throw), so greed gets punished. Arm a 🟣 MASTER BALL for a guaranteed catch — or take a LEGENDARY head-on: it fights as a 🩸 RAID BOSS with a huge segmented health bar, and the deeper you carve it the higher your throw odds climb (~85% at the brink): you earn 5 for every Champion you beat and more for nuzlocke crowns (3 each, 5 for ultimates). ✨ 1-in-20 encounters are SHINY — different colors, DOUBLE payout, and their OWN Pokédex. 🧭 THE GEN LADDER: your wild starts at Gen 1 (#1–151) and BATTLES open the rest — beat Kanto's Champion to spill Gen 2 into the grass, then each Champion you beat in The Journey unlocks the next generation (LANCE→Gen 3, STEVEN→Gen 4… LEON→Gen 9, all the way to #1025). No dex completion required — it's about the climb. You won't re-meet something you've already caught, but a shiny counts separately — catch a normal Totodile and a shiny one can still turn up. Flip the Regular ⇄ ✨ Shiny tabs in your dex. ~30% of catches drop a 🍓 Sitrus Berry (auto-heals in duels), tap any caught mon for nicknames — and every so often, with the room synced, the sky breaks and a 🌩 ROAMING LEGENDARY appears — a room-wide RACE where the FIRST catch claims it (a 🌩 Storm Chaser point on the wall). Everything else: everyone builds their own dex, no contention." },
