...
(function () {
  ...
  function spriteOf(id) { return SP[id] || Store.sprite(id); }

  // Cinematic swap overlay: sprites cross, evolutions flash.
  function playTrade(aName, aMon, bName, bMon, result, onDone) {
    const lay = el("div", { class: "trade-anim" }, [
      el("div", { class: "trade-anim-row" }, [
        el("img", { class: "trade-fly left", src: spriteOf(aMon), alt: "" }),
        el("div", { class: "trade-arrows" }, "⇄"),
        el("img", { class: "trade-fly right", src: spriteOf(bMon), alt: "" }),
      ]),
    ...
    if (result.evoB) steps.push([1400, () => {
      ...
      const img = lay.querySelector(".trade-fly.left"); if (img) { img.src = spriteOf(result.toB); img.classList.add("evo"); }
      sfx("evolve");
    }]);
    if (result.evoA) steps.push([1400, () => {
      txt.textContent = "⚡ Whoa! The traded " + nameOf(bMon) + " is evolving… it became " + nameOf(result.toA) + "!";
      const img = lay.querySelector(".trade-fly.right"); if (img) { img.src = spriteOf(result.toA); img.classList.add("evo"); }
      sfx("evolve");
    }]);
    ...
    function sideCard(key, title, cls) {
      ...
      if (d.attId) {
        ...
        kids.push(el("div", { class: "duel-pick-row" }, pool.map((id) => {
          ...
          return el("button", { class: "duel-pick" + (d.mon === id ? " on" : "") + (ok ? "" : " locked"),
            title: nameOf(id) + (locked ? " — partner, untradeable ❤" : ""),
    ...
    function render() {
      ...
      host.appendChild(el("div", { class: "toolbar", style: { justifyContent: "center" } }, [
        el("button", { class: "btn spin-btn", onClick: () => {
          ...
          playTrade(an, aM, bn, bM, result, () => { render(); renderRecent(); });
        } }, "🔁 MAKE THE TRADE"),
      ]));
    }
